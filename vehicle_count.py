...
cap = cv2.VideoCapture('Film/Test_video.m4v')
input_size = 320
...
def realTime():
    while True:
        ...
        ### ZELF ERBIJ GEZET VIA https://github.com/hybridgroup/gocv/issues/388  ###
...
def from_static_image(image):
    ...
    import _strptime; (_strptime)
    # parse local date
    # switch back to previous locale
    tijd=datetime.now().strftime('%H:%M:%S.%f')[:-2]
    dag= datetime.today().strftime("%A")
    maand= datetime.now().strftime("%B")
    datum= datetime.today().strftime('%Y-%m-%d')
    jaar= datetime.today().strftime('%Y')
    print(datum,tijd , dag, maand,jaar, get_season(datetime.today()))
    locale.setlocale(locale.LC_TIME, old_loc)
    with open("static-data.csv", 'a') as f1:

        cwriter = csv.writer(f1)

        cwriter.writerow([image, frequency['car'], frequency['motorbike'], frequency['bus'], frequency['truck'],datum,tijd,dag,maand,jaar])
